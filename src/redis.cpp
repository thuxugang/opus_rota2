...
void Redis::addAllRotamers(Residue& residue){
...
void Redis::addAllDASFs(string& key, vector<Residue>& residuesData, vector<Geo*>& geosData, int center, int half, int window_len){
...
void addRotamersInfo(vector<Residue>& residuesData, Redis& r){
	...
	for(int i=0; i<length; i++){
		r.addAllRotamers(residuesData[i]);
	}
}
...
void addDASFInfo(vector<Residue>& residuesData, vector<Geo*>& geosData, Redis& r){
	...
	for (int i = 0; i<length; i++){
		...
		for (int j = half_1; j< length_seq-half_1; j++){
			...
			r.addAllDASFs(key, residuesData, geosData, j, half_1, window_len);
		}	
	}
}
