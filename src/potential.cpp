...
void initAllPotentials(vector<Geo*>& geosData, vector<Residue>& residuesData){
	...
	for(int i=0; i<seq_length; i++){
		...
		for(int j=0; j<rl_length; j++){
			...
			//rotamer lib score
			residuesData[i].m_rotamerlibs[j].m_RLScores = stod(CONFIG["weigh_rp"])*getRLScore(residuesData[i], j);
			//main chain - side chain LJ potential
			residuesData[i].m_rotamerlibs[j].m_MS_LJScores = stod(CONFIG["weight_mslj"])*getMSLJPotential(residuesData, i, j);
			//DASF score
			residuesData[i].m_rotamerlibs[j].m_DASFScores = stod(CONFIG["weight_dasf"])*getDASFPotential(residuesData, i, j);
			//total
